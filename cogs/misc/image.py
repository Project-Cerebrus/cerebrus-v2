...
class Images(commands.Cog):
	def __init__(self, bot: commands.Bot):
		self.bot = bot
	@commands.command()
	async def avurl(self,ctx,user:discord.Member=None):
		if user == None:
			user = ctx.author
		...
		embed = discord.Embed(title=f"{user.name}'s Av'",description = f"[Click here]({url})",color=ctx.author.color)
		await ctx.send(embed=embed)
	@commands.command(name='fry',aliases=['deepfry'])
	async def fry(self, ctx: commands.Context,help="",amount="20"):
		if help == discord.Member:
			await ctx.reply("Please type avurl and put that url here")
			return
		'''
		help = help.replace("<@!","")
		help = help.replace(">","")
		help = await self.bot.get_user(int(help))
		help = str(help.avatar_url)
		help = help.replace('webp', 'jpg')
		'''
	...
	@commands.command(name='magik',aliases=['magicimg'])
	async def magik(self, ctx: commands.Context,help=""):
		amount = 1
		if help == discord.Member:
			await ctx.reply("Please type avurl and put that url here")
			return
