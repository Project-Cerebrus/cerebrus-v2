...
class Music(commands.Cog):
    ...
    async def connect_(self, ctx, *, channel: discord.VoiceChannel=None):
        ...
        if check == "enabled":
          print("[Music]: enabled")
        if check == "disabled": # how do I get avatar url?
          await ctx.send("package disabled, use mpkg to reinstall")
          return
        """Connect to voice.
        Parameters
        ------------
        ...
        if not channel:
            try:
                ...
                embed = discord.Embed(title="", description="No channel to join. Please call `<p>join` from a voice channel.", color=discord.Color.green())
                await ctx.send(embed=embed)
        ...
        embed = discord.Embed(title = 'Joining Channel', description = f'`{channel}`', colour = discord.Color.green())
        await ctx.send(embed=embed)
    ...
    async def spotify(self, ctx, *, query):
        x = await funcs.search_spotify("track", query)
        print(x)
